package("localopencv")
    ...
    on_load("android", "linux", "macosx", function (package)
        ...
        if package:config("tesseract") then
            package:add("deps", "tesseract 4.1.3")
        end
    end)
    ...
    on_install("windows", function (package)
        local url = "https://github.com/opencv/opencv/releases/download/4.12.0/opencv-4.12.0-windows.exe"
        local filename = path.filename(url)
        local cachedir = package:cachedir()
        local exe_path = path.join(cachedir, filename)
        if not os.isfile(exe_path) then
            print("Downloading OpenCV Windows prebuilt package ...")
            import("net.http").download(url, exe_path)
        end
        ...
        if not os.isfile(marker) then
            print("Extracting OpenCV Windows prebuilt package ...")
            local sevenzip = path.join(os.scriptdir(), "7zr.exe")
            os.execv(sevenzip, {"x", exe_path, "-o" .. windowsdir})
        else
            print("OpenCV already extracted, skip extracting.")
        end
    end)
